...
func (cs *ConfigService) Load() ConfigResponse {
...
func (cs *ConfigService) Save(authToken, baseURL string) ConfigResponse {
	configPath := cs.GetConfigPath()
	...
	if existing.Success {
		...
		if existingData, ok := existing.Data.(ClaudeConfig); ok {
			config = existingData
		}
	}
